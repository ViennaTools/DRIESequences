...
public:
  ...
  void setLineDistance(T distance) {
  ...
  void apply() {
    ...
    while(maskVec[1] < domainBounds[3]) {
      ...
      lsBooleanOperation<T, 3>(mask, maskSpot, lsBooleanOperationEnum::UNION).apply();
      ...
      if(maskVec[0] > domainBounds[1]) {
        maskVec[0] = domainBounds[2] + (((yLines++)%2)?1.5:0.5) * (lineDistance + 2 * maskRadius);
    ...
    lsMakeGeometry<T, 3>(maskBottom, lsSmartPointer<lsPlane<T, 3>>::New(maskOrigin.data(), axis)).apply();
    substrate->deepCopy(maskBottom);
    lsBooleanOperation<T, 3>(substrate, mask, lsBooleanOperationEnum::UNION).apply();
    lsBooleanOperation<T, 3>(maskBottom).apply(); // invert
    lsBooleanOperation<T, 3>(mask, maskBottom, lsBooleanOperationEnum::INTERSECT).apply();
  }
};